...
def compress_zip():
    return render_template("create-zip-file.html")
...
app.config["FILE_UPLOADS_TEMP"] = "/home/panashe/file-compression-tool/temp"

@app.route("/compresszip", methods=["GET", "POST"])
def compresszip():
    if request.method == "GET":
        return render_template("compress-zip-file.html", check=0)
    else:
        ...
        if len(up_file.filename) > 0:
            ...
            os.system('./zipper compress temp zipped.zip'.format(filename))            
...
def compressdoc():
    if request.method == "GET":
        ...
        if len(up_file.filename) > 0:
            ...
            print(up_file.filename)
            up_file.save(os.path.join(app.config["FILE_UPLOADS"], filename))
            os.system('./compress_doc uploads/{}'.format(filename))            
            filename = filename[:filename.index(".",1)]
            ftype = "_compressed.bin"
            while True:
                if 'uploads/{}_compressed.bin'.format(filename) in glob.glob('uploads/*_compressed.bin'):
